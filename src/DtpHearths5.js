...
export const DtpHearths5 = L.featureGroup([]);
...
// console.log('DtpHearths.setFilter ', arg, DtpHearths._group);
	...
	if (DtpHearths5._group) {
		...
		DtpHearths5.addLayer(L.layerGroup(arr));
						...
						it._bounds
							...
							.on('click', (ev) => {
								...
								if (dist < 10) {
									...
									setPopup1(it);
									popup1.setLatLng(latlng).openOn(DtpHearths5._map);
								}
...
// console.log('opt', opt);
				...
				DtpHearths5._group = L.layerGroup(arr);
				if (argFilters) {
					DtpHearths5.setFilter(argFilters);
				} else {
